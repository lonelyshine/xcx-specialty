...
import com.chunyang.util.HttpConstants;
import com.chunyang.util.JsonDateValueProcessor;
...
/** 
 * Controller基类 ,主要是对service返回的数据类型进行json处理，以便可以返回到前端
 */  
public class BaseController {  
     ...
     * @return 输出成功的JSON格式数据 
     */  
    /*public String responseMapArraySuccess(HashMap<String,Object> obj){  
        JSONArray jsonObj = null;  
        if(obj != null){  
            logger.info("后端返回对象：{}", obj);  
            JsonConfig jsonConfig = new JsonConfig();  
     ...
     * @return 输出成功的JSON格式数据 
     */  
    /*public String responseSuccess(HashMap<String,Object> obj, String msg){  
        JSONObject jsonObj = null;  
        if(obj != null){  
            logger.info("后端返回对象：{}", obj);  
            JsonConfig jsonConfig = new JsonConfig();   
